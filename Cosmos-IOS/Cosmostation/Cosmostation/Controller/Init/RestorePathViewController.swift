...
class RestorePathViewController: BaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        } else if (userChain == ChainType.BAC_MAIN) {
            cell?.pathLabel.text = BAC_BASE_PATH.appending(String(indexPath.row))
        ...
        DispatchQueue.global().async {
            ...
            DispatchQueue.main.async(execute: {
                ...
                } else if (self.userChain == ChainType.BAC_MAIN) {
                    cell?.denomAmount.attributedText = WUtils.displayAmount2(NSDecimalNumber.zero.stringValue, cell!.denomAmount.font!, 0, 8)
                    let request = Alamofire.request(BAC_LCD_URL_ACCOUNT_INFO + address, method: .get, parameters: [:], encoding: URLEncoding.default, headers: [:])
                    request.responseJSON { (response) in
                        switch response.result {
                        case .success(let res):
                            guard let info = res as? [String : Any] else {
                            ...
                            let bacAccountInfo = BacAccountInfo.init(info)
                            for bacBalance in bacAccountInfo.balances {
                                if (bacBalance.denom == BAC_MAIN_DENOM) {
                                    cell?.denomAmount.attributedText = WUtils.displayAmount2(bacBalance.amount, cell!.denomAmount.font!, 0, 8)
                                }
                            }
                        case .failure(let error):
                            if (SHOW_LOG) { print("onFetchAccountInfo ", error) }
