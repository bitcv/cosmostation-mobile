...
import wannabit.io.bitcv.activities.RewardAddressChangeActivity;
...
import static wannabit.io.bitcv.base.BaseConstant.FEE_AKASH_GAS_AMOUNT_ADDRESS_CHANGE;
import static wannabit.io.bitcv.base.BaseConstant.FEE_AKASH_GAS_RATE_AVERAGE;
import static wannabit.io.bitcv.base.BaseConstant.FEE_CERTIK_GAS_ADDRESS_CHANGE;
import static wannabit.io.bitcv.base.BaseConstant.FEE_CERTIK_GAS_RATE_AVERAGE;
import static wannabit.io.bitcv.base.BaseConstant.FEE_IOV_GAS_AMOUNT_LOW;
import static wannabit.io.bitcv.base.BaseConstant.FEE_IOV_GAS_RATE_AVERAGE;
import static wannabit.io.bitcv.base.BaseConstant.SECRET_GAS_AMOUNT_REWARD_ADDRESS_CHANGE;
...
public class RewardAddressChangeStep2Fragment extends BaseFragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (getSActivity().mBaseChain.equals(BaseChain.COSMOS_MAIN)) {
            ...
            mGasAmount.setText(BaseConstant.FEE_IRIS_GAS_AMOUNT_ADDRESS_CHANGE);
            mGasRate.setText(WDp.getDpString(BaseConstant.FEE_IRIS_GAS_RATE_AVERAGE, 6));
            mFeeAmount = new BigDecimal(BaseConstant.FEE_IRIS_GAS_AMOUNT_ADDRESS_CHANGE).multiply(new BigDecimal(BaseConstant.FEE_IRIS_GAS_RATE_AVERAGE)).movePointRight(18).setScale(0);
            ...
            mGasFeeAmount.setText(WDp.getDpString(WDp.attoToIris(mFeeAmount).setScale(2).toPlainString(), 2));
            ...
            mGasAmount.setText(SECRET_GAS_AMOUNT_REWARD_ADDRESS_CHANGE);
            mGasRate.setText(WDp.getDpString(SECRET_GAS_FEE_RATE_AVERAGE, 3));
            mFeeAmount = new BigDecimal(SECRET_GAS_AMOUNT_REWARD_ADDRESS_CHANGE).multiply(new BigDecimal(SECRET_GAS_FEE_RATE_AVERAGE)).setScale(0);
            if(getBaseDao().getCurrency() != 5) {
            ...
            mGasAmount.setText(FEE_AKASH_GAS_AMOUNT_ADDRESS_CHANGE);
            mGasRate.setText(WDp.getDpString(FEE_AKASH_GAS_RATE_AVERAGE, 3));
            mFeeAmount = new BigDecimal(FEE_AKASH_GAS_AMOUNT_ADDRESS_CHANGE).multiply(new BigDecimal(FEE_AKASH_GAS_RATE_AVERAGE)).setScale(0);
            if(getBaseDao().getCurrency() != 5) {
    ...
    public void onClick(View v) {
        if(v.equals(mBeforeBtn)) {
            ...
            if (getSActivity().mBaseChain.equals(BaseChain.COSMOS_MAIN)) {
                ...
                amount.add(gasCoin);
                fee.amount = amount;
                fee.gas = BaseConstant.FEE_IRIS_GAS_AMOUNT_ADDRESS_CHANGE;
                getSActivity().mFee = fee;
