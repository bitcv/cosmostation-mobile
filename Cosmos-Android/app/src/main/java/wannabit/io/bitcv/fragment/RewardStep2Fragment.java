...
import static wannabit.io.bitcv.base.BaseConstant.FEE_IRIS_GAS_AMOUNT_REWARD_MUX;
...
public class RewardStep2Fragment extends BaseFragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (getSActivity().mBaseChain.equals(COSMOS_MAIN)) {
            ...
            mEstimateGasAmount = (new BigDecimal(FEE_IRIS_GAS_AMOUNT_REWARD_MUX).multiply(new BigDecimal(""+getSActivity().mValidators.size()))).add(new BigDecimal(BaseConstant.FEE_IRIS_GAS_AMOUNT_REWARD_BASE));
            mGasAmount.setText(mEstimateGasAmount.toPlainString());
            mGasRate.setText(WDp.getDpString(FEE_IRIS_GAS_RATE_AVERAGE, 6));

            mFeeAmount = mEstimateGasAmount.multiply(new BigDecimal(FEE_IRIS_GAS_RATE_AVERAGE)).movePointRight(18).setScale(0);
            ...
            mFeeAmount = mEstimateGasAmount.multiply(new BigDecimal(FEE_IOV_GAS_RATE_AVERAGE)).setScale(0);
            if(getBaseDao().getCurrency() != 5) {
            ...
            mFeeAmount = mEstimateGasAmount.multiply(new BigDecimal(FEE_AKASH_GAS_RATE_AVERAGE)).setScale(0);
            if(getBaseDao().getCurrency() != 5) {
                mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastCertikTic())).setScale(2, RoundingMode.DOWN);
            } else {
                mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastCertikTic())).setScale(8, RoundingMode.DOWN);
    ...
    private void onUpdateFeeLayer() {
        if (getSActivity().mBaseChain.equals(COSMOS_MAIN)) {
            if(mSeekBarGas.getProgress() == 0) {
                ...
                mFeeAmount = mEstimateGasAmount.multiply(new BigDecimal(FEE_GAS_RATE_LOW)).setScale(0);
                if(getBaseDao().getCurrency() != 5) {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastAtomTic())).setScale(2, RoundingMode.DOWN);
                } else {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastAtomTic())).setScale(8, RoundingMode.DOWN);
                ...
                mFeeAmount = mEstimateGasAmount.multiply(new BigDecimal(FEE_GAS_RATE_AVERAGE)).setScale(0);
                if(getBaseDao().getCurrency() != 5) {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastAtomTic())).setScale(2, RoundingMode.DOWN);
                } else {
                    mFeePrice = WDp.uAtomToAtom(mFeeAmount).multiply(new BigDecimal(""+getBaseDao().getLastAtomTic())).setScale(8, RoundingMode.DOWN);
